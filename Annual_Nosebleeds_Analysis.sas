...
proc genmod data=baseline_efficacy_info;
	...
	model nosebleeds=arm tissue_use  previous_year country /dist=poisson offset=duration_year type1; *offset is to adjust the non-365 days patients ;
...
proc genmod data=baseline_efficacy_info;
	where mucus_viscosity>=1 and tissue_use="HIGH";
	class arm tissue_use country;
	model nosebleeds=arm tissue_use  previous_year country /dist=poisson offset=duration_year type1 ; 
	output out=out_poi pred=pred_poi;
run;
